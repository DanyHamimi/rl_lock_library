...
int main() {
    ...
    rl_descriptor descriptor = rl_open("exSujet.txt", O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (descriptor.d == -1) {
    ...
    rl_descriptor descriptor2 = rl_open("exSujet.txt", O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (descriptor2.d == -1) {
        perror("Erreur lors de l'ouverture du fichier");
        return 1;
    }
